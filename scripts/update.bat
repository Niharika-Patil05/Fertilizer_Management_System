...
where docker >nul 2>nul || (echo [FAILED] Docker is not installed. & pause & exit /b 1)
docker info >nul 2>nul || (echo [FAILED] Docker Desktop is not running. & pause & exit /b 1)
if not exist ".env" (echo [FAILED] .env not found. Run scripts\start.bat once first. & pause & exit /b 1)
...
if "%NEW%"=="" (
  ...
  for /f "usebackq delims=" %%N in (`powershell -NoProfile -Command "try { (Invoke-RestMethod -UseBasicParsing '%REPO_RAW%').version } catch { '' }"`) do set "NEW=%%N"
)
if "%NEW%"=="" (echo [FAILED] Could not determine the target version. Connect to the internet or run: scripts\update.bat ^<version^> & pause & exit /b 1)
echo Target version: %NEW%

if "%NEW%"=="%OLD%" (echo [SUCCESS] Already running version %OLD%. Nothing to do. & pause & exit /b 0)

echo.
echo === Step 1/5: Backup ===
call "%~dp0backup.bat" quiet
if errorlevel 1 (echo [FAILED] Backup failed - update aborted. Nothing was changed. & pause & exit /b 1)
set "BACKUP_DIR="
